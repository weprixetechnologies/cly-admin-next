...
export default function BlogTagsPage() {
    ...
    const loadTags = async () => {
        try {
            setLoading(true);
            const res = await axiosInstance.get('/blog/tags');
            if (res.data?.success) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editId) {
                const res = await axiosInstance.put(`/admin/blog/tags/${editId}`, payload);
                if (res.data?.success) {
                ...
                const res = await axiosInstance.post('/admin/blog/tags', payload);
                if (res.data?.success) {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Are you sure you want to delete this tag? Links to existing blog posts will be removed.')) {
            return;
        }

        try {
            const res = await axiosInstance.delete(`/admin/blog/tags/${id}`);
            if (res.data?.success) {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            <div>
                <h1 className="text-3xl font-extrabold text-gray-900 font-serif tracking-tight">Blog Tags</h1>
                <p className="text-gray-500 text-sm mt-1">Manage keywords/tags linked to blog articles.</p>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                {/* Left side: Tags Table */}
                <div className="lg:col-span-7 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
                        <h2 className="font-bold text-gray-900 text-sm uppercase tracking-wider">Existing Tags</h2>
                    </div>

                    {loading ? (
                        <div className="p-8 text-center text-gray-500">Loading tags...</div>
                    ) : tags.length === 0 ? (
                        <div className="p-8 text-center text-gray-400">No tags found. Create one using the form.</div>
                    ) : (
                ...
                <div className="lg:col-span-5 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-3 justify-end pt-2">
                            ...
                            >
                                {submitting ? 'Saving...' : editId ? 'Update Tag' : 'Create Tag'}
                            </button>
                        </div>
                    </form>
                </div>
